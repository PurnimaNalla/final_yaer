...
const AwardsSection = () => (
  <section id="awards" className="py-16 bg-[#FAF3E0]">
    <div className="max-w-4xl mx-auto px-6">
      <h2 className="text-4xl font-bold text-center text-[#3B185F] mb-12">
        Awards and Recognitions
      </h2>

      <div className="relative border-l-4 border-[#3B185F] pl-6">
        {awardsData.map((award, index) => (
          <div
            key={index}
            className="mb-10 relative group transition-all duration-300 ease-in-out hover:scale-105"
          >
            {/* Timeline Dot */}
            <div className="absolute -left-[22px] top-2 w-6 h-6 bg-[#3B185F] rounded-full border-4 border-white group-hover:scale-125 transition-transform duration-300"></div>

            {/* Award Details */}
            <div className="bg-white shadow-md rounded-lg p-6">
              <h3 className="text-xl font-bold text-[#3B185F]">{award.year}</h3>
...
const clientsData = [
  ...
  { 
    image: "public/images/image20.jpg", 
    name: "2008",
  ...
  const ClientsSection = () => (
    <section id="client" className="py-16 bg-white">
      <div className="max-w-7xl mx-auto px-6">
        <h2 className="text-3xl font-bold text-blue-900 text-center mb-8">Our Clients</h2>
        <Swiper
          spaceBetween={30}
          slidesPerView={2}
          navigation={true}
        ...
        >
          {clientsData.map((client, index) => (
            <SwiperSlide key={index} className="flex flex-col items-center text-center">
              <img src={client.image} alt={client.name} className="h-20 w-auto mb-4" />
              <h3 className="text-xl font-bold text-[#3B185F]">{client.name}</h3>
...
function About() {
  return (
    <div className="bg-gray-50 mt-20">
       <section className="relative bg-gradient-to-r from-blue-500 to-pink-500 text-white py-20">
      <div className="absolute top-0 left-0 w-full h-full bg-blue-400 skew-y-3 transform origin-top"></div>
        <div className="relative max-w-7xl mx-auto px-6 sm:px-12 lg:px-20 flex items-center">
          ...
          <div className="w-1/2">
            <img src="/images/about2.png" alt="About Us" className="rounded-lg shadow-md" />
          </div>
        </div>
      </section>

   {/* Company Background */}
   <section id="company-background" className="relative py-16 bg-white">
     ...
     {/* Our Journey */}
     <section id="our-journey" className="relative py-16 bg-gradient-to-r from-blue-500 to-pink-500 text-white">
        ...
        <div className="relative max-w-7xl mx-auto px-6 sm:px-12 lg:px-20">
          <h2 className="text-3xl font-bold mb-6 text-center">Our Journey Throughout the Decades!</h2>
          <div className="flex gap-8 overflow-x-auto py-8">
            {[
              ...
              <div
                key={index}
                className="min-w-[300px] bg-white text-blue-900 rounded-lg shadow-md p-4 flex flex-col items-center justify-center hover:scale-105 transform transition duration-300"
              >
...
<section id="management-team" className="py-16 bg-gray-100">
  <div className="max-w-7xl mx-auto px-6">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-10">
      {[
        ...
        <div
          key={index}
          className="bg-white rounded-lg shadow-lg p-6 flex flex-col items-center text-center transition-transform duration-300 ease-in-out hover:shadow-2xl hover:-translate-y-3"
        >
          <img
            src={member.image}
            alt={member.name}
